...
public class BoyaGrandeRepository {
    ...
    public static final int SIZE_BUFFER_STREAM = 35 * 1024 * 1024;
    ...
    public String getToken() {
        ...
        TokenResponse tokenResponse = client.post().uri(uriBuilder -> uriBuilder.path(pathBase).path("/auth/login").build()).
    ...
    public BoyaGrande getBoyaUltimoTrack(String token, long idBoya) {
        ...
        return client.get().uri(uriBuilder -> uriBuilder.path(pathBase).path("/tracks/device/").path(String.valueOf(idBoya)).
                build()).header(HttpHeaders.AUTHORIZATION, BEARER + token).retrieve().bodyToMono(BoyaGrande.class).block();
    }
    ...
    public BoyaGrandeTracks getBoyaTracks(String token, long idBoya) {
        ...
        return client.post().uri(uriBuilder -> uriBuilder.path(pathBase).path("/viewdata").build()).
                header(HttpHeaders.AUTHORIZATION, BEARER + token).contentType(MediaType.APPLICATION_JSON).
                bodyValue(boyaGrandeTrackRequest).retrieve().bodyToMono(BoyaGrandeTracks.class).block();
    }
    ...
    public Mono<BoyaGrande> getBoyaUltimoTrackMono(String token, long idBoya) {
        WebClient client = WebClient.create(url);
        return client.get().uri(uriBuilder -> uriBuilder.path(pathBase).path("/tracks/device/").path(String.valueOf(idBoya)).
                build()).header(HttpHeaders.AUTHORIZATION, BEARER + token).retrieve().bodyToMono(BoyaGrande.class);
    }
